...
const levels = [
	...
	},
	{
		...
		task   = "Создайте суперпозицию [i]|001⟩[/i] и [i]|011⟩[/i]",
		input  = [1,0,1],
		prob   = true,
		output = [
			Vector2(0,0), #000
	...
	},
	{
		id     = 11,
		desc   = "entanglement",
		task = "Используя управляющий узел, создайте запутанную пару кубитов с одинаковым состоянием " + \
		"(суперпозиция [i]|00⟩[/i] и [i]|11⟩[/i] )\n\n" + \
		"[b]Подсказка:[/b]\nпри работе с суперпозициями вы можете рассматривать каждое базисное состояние как свой собственный случай или свою собственную вселенную. " + \
		"активация управляемого вентиля зависит только от состояния управляющего бита в каждом отдельном случае/вселенной.",
		input  = [0,0],
		prob   = true,
		output = [
	...
	},
	{
		id     = 12,
		desc   = "entanglement",
		task = "Создать запутанную пару кубитов с противоположным состоянием " + \
		"(суперпозиция [i]|01⟩[/i] и [i]|10⟩[/i] )",
		input  = [0,0],
		prob   = true,
		output = [
			Vector2(0,0), #00
	...
	},
	{
		id     = 13,
		desc   = "phase intro",
		task   = "Верните кубит обратно в состояние [i]|0⟩[/i]",
		input  = [0],
		prob   = true,
		output = [
			Vector2(1,0), #0
	...
	},
	{
		id     = 14,
		desc   = "phase again",
		task   = "[b]Input : [/b] [i]|ψ⟩ = ( |0⟩ - |1⟩ )/sqrt(2)[/i]\n\nВерните кубит в состояние [i]|0⟩[/i]",
		input  = null,
		nQ     = 1,
		istate = [
			Vector2(0.707107,0), #0
	...
	},
	{
		...
		task = "[b]Вход: [/b] Входное состояние имеет 30% вероятность [i]|0⟩[/i]" + \
		"и 70% вероятность [i]|1⟩[/i]\n\n" + "измените его так, чтобы он имел " + \
		"30% вероятность [i]|1⟩[/i] и 70% вероятность [i]|0⟩[/i]",
		input  = null,
		nQ     = 1,
		istate = [
	...
	},
	{
		id     = 16,
		desc   = "phase flip",
		task   = "Превратите кубит в состояние [i](|0⟩ - |1⟩)/sqrt(2)[/i]",
		input  = [0],
		prob   = false,
		output = [
	...
	},
	{
		...
		task   = "Измените кубит из состояние  [i]|0⟩[/i] в [i]|1⟩[/i] используя только " + \
		"Гейт Адамара и Паули-Z",
		input  = [0],
		prob   = true,
		output = [
			Vector2(0,0), #0
	...
	},
	{
		id     = 18,
		desc   = "Y gate",
		task   = "Создайте состояние ( [i]–i|0⟩ – i|1⟩[/i] )/sqrt(2)",
		input  = [0],
		prob   = false,
		output = [
